...
class Network():
    ...
    def draw(self):
        ...
        # Desenhar nós com cores
        node_colors = [self.G.nodes[n]['color'] for n in self.G.nodes]
        nx.draw_networkx_nodes(self.G, pos, node_color=node_colors, node_size=500)
        node_indices = {node: node for node in self.G.nodes}
        nx.draw_networkx_labels(self.G, pos, labels=node_indices, font_color='white')

        # Peso das arestas
        edge_weights = [self.channels.get((u, v), {}).get("fidelity_value", None) for u, v in self.G.edges]

        # Filtrar arestas com valores de fidelidade válidos
        valid_edges = [(u, v) for (u, v), weight in zip(self.G.edges, edge_weights) if weight is not None]
        valid_weights = [weight for weight in edge_weights if weight is not None]

        # Definir cores das arestas com base nos valores de fidelidade
        edge_color_map = plt.cm.ScalarMappable(cmap=plt.cm.viridis, norm=plt.Normalize(vmin=min(valid_weights), vmax=max(valid_weights)))
        edge_colors = [edge_color_map.to_rgba(weight) for weight in valid_weights]

        # Desenhar arestas com cores e larguras definidas
        nx.draw_networkx_edges(self.G, pos, edgelist=valid_edges, width=2, edge_color=edge_colors)

        # Adicionar uma barra de cores
        cbar = plt.colorbar(edge_color_map, ax=plt.gca(), label="Fidelidade")
        ...
        for qubit in qubits:
            ...
            for indice in range(len(route) - 1):
                ...
                if random.uniform(0, 1) > channel_fidelity:
                    # A qubit sofreu interferência
                    index_interference_qubit.add(qubits.index(qubit))
            # Se o qubit sofreu interferência, não chegará no destino
            if qubits.index(qubit) not in index_interference_qubit:
